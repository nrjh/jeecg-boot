...
<!-- 查看按钮弹出表单 备品备件查看页面-->
  <a-modal
    :title="title1"
    :width="width"
    :visible="visibleLookSparePartsModel"
          ...
          <a-col :span="24">
            <a-form-item label="货位" :labelCol="{span:2}" :wrapperCol="wrapperCol">
              <JDictSelectTagIop
                placeholder=""
                disabled="disabled"
                dictCode="stk_location,complete_name,id" v-model='locationId' style="width: 100%"/>
            </a-form-item>
          </a-col>
        </a-row>
      </div>
...
<!--            <a-input v-model="remark" placeholder="" rows="4" disabled="disabled"></a-input>-->
<!--          </a-form-item>-->
<!--        </a-col>-->
<!--      </a-row>-->
      <a-row :gutter="24">
        <a-col :span="24">
          <a-form-item :labelCol="{span:1}" :wrapperCol="{span:24}">
            <a-textarea  v-model="remark" placeholder="请输入备注" :rows="4" disabled="disabled"/>
          </a-form-item>
        </a-col>
      </a-row>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      stockWarn: '',
      remark: '',
      visibleLookSparePartsModel: false,
      labelCol: {
        span: 6
      },
      wrapperCol: {
      ...
      url: {
        find:"/spareparts/stkSpareParts/queryById",//主表 表单回显数据
      },
    };
  },
  methods: {
    handleCancelCheck(){
      this.visibleLookSparePartsModel=false;
    },
    handleOKCheck(){
      this.visibleLookSparePartsModel=false;
    },
   showCheck(sparePartsId){
     this.visibleLookSparePartsModel=true;
     ...
     getAction(this.url.find,{id:sparePartsId}).then((res)=>{
       if(res.success){
         ...
         this.joinParts=res.result.joinParts_dictText;
