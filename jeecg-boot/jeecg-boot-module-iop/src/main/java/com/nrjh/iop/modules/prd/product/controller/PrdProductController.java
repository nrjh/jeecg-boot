...
public class PrdProductController {
    ...
    @AutoLog(value = "物品-关联分页列表查询")
    @ApiOperation(value = "物品-关联分页列表查询", notes = "物品-关联分页列表查询")
    @GetMapping(value = "/ref/list")
    public Result<?> refQueryPageList(RefQuery refQuery,
                                      @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                      @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize) {
        Page<PrdProduct> page = new Page<PrdProduct>(pageNo, pageSize);
        return Result.ok(prdProductService.refPage(page, refQuery));
    }
